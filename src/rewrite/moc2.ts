...
function glued(node: NormalBranch): boolean {
    // A spaced `(`/`[` after a head starts the branch instead of continuing the head.
    if (node.kind === 'call_exp' || node.kind === 'array_idx_exp') {
        const [, second] = node.children;
        if (second?.nodeType === 'Text') return false;
    }
    return node.children.every((c) => c.nodeType !== 'Branch' || glued(c));
}

function headSafe(inner: NormalBranch): boolean {
    if (!HEAD_KINDS.has(inner.kind)) return false;
    // `{` in a head is read as a record, and a prefix-shaped `-`/`+`/`^`/`#` starts the branch.
    if (inner.text.includes('{') || /\s[-+^#][^\s=]/.test(inner.text))
        return false;
    return glued(inner);
}
