...
//
// Exit code is 1 when the expected-count check fails on either set, 0 otherwise. Parsing failures are
// reported but do not fail the run (the plan says they are recorded, not fatal).

import { BASE, inOracleSet } from '../lib/config.mjs';
import { assertCheckout, listFiles } from '../lib/git.mjs';
import { census, checkExpected } from '../lib/census.mjs';
...
async function main() {
    ...
    const out = {
        ...
        checks: { oracle: oracleCheck, full: fullCheck },
    };
    ...
    process.exit(out.checks.oracle.ok && out.checks.full.ok ? 0 : 1);
}
