...
- Install: `npm ci` (requires prettier as a peer dependency: `npm i prettier`).
...
- CI (`.github/workflows/tests.yml`) runs on Node 18, 20, and 22, and clones `https://github.com/dfinity/motoko` into `../motoko` (a sibling of this repo) before testing. The compiler-suite test reads Motoko test files from that path.
